...
class DioHelper {
  ...
  static Future init() async {
    ...
    String userLocale = getx.Get.find<Lang2Controller>().selectedLang;
  ...
  static Future dataToFormData({
    ...
    if (files is Map && files.length > 0) {
      /// Set attribute name
      fromDataAttributeName = fromDataAttributeName == null ? 'files' : fromDataAttributeName;
      ...
      /// Set attribute name
      fromDataAttributeName = fromDataAttributeName == null ? 'file' : fromDataAttributeName;
  ...
  static Future get({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        showError(body: response.data['error_message']);
        return false;
      }
    } on DioError catch (e) {
      if (e.error is SocketException || e.type == DioErrorType.connectTimeout) {
        showError(body: 'errorInternet');
        return false;
      }
    } catch (e) {
  ...
  static Future post({
    ...
    try {
      ...
      print('saqr$response');
      print('saqr${response.statusCode}');
      dynamic dynamicResponse = await jsonDecode(response.toString());
      if (response.statusCode == 200) {
        return dynamicResponse;
      } else {
        showError(body: response.data['message']);
        return false;
      }
    } on DioError catch (e) {
      if (e.error is SocketException || e.type == DioErrorType.connectTimeout) {
